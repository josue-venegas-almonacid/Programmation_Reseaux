               ...
               else
               {
                  ...
                  if (buffer[0] == '/'){
                     if (strcmp(buffer, "/disconnect") == 0){
                        ...
                        // only can create a party if the user is in the lobby
                        
                        printf("Client %s created a party\n", client.name);
                        continue;
                        
                     }

                     else if (strcmp(buffer, "/list_parties") == 0){
                        // only can list parties if the user is in the lobby
                        continue;
                     }

                     else if (strcmp(buffer, "/start_game") == 0){
                        printf("Client %s try to start a game\n", client.name);
                        printf("Party_id: %s\n", party_id);
                        
                        // only can start a game if there are 2 players in the party
